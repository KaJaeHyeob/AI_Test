...
## 모델 설정
model = Sequential()
model.add(Dense(500, input_dim=len(train_x.columns), activation='relu'))
...
model.fit(x_train, y_train, batch_size=5000, validation_split=0.2,
...
## 모델 저장
model.save('code/my_model_.h5')

## 모델 삭제
del model

## 모델 불러오기
model = load_model('code/my_model_3802.h5')
