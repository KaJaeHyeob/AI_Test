...
train = train.drop(columns=['type_num'])
...
all_FID = all_data[['fiberID']].astype('category')
...
'''
all_data = pd.read_csv("dataset/all_data.csv", index_col='id')
all_data = pd.concat([all_FID, all_data], axis=1)
...
## 모델 설정
model = Sequential()
model.add(Dense(100, input_dim=len(train_x.columns), activation=mish))
...
## LGBM 모델 설정, 실행, 예측
model = lgbm.LGBMClassifier(n_estimators=1024, num_leaves=128, learning_rate=0.01, random_state=1)
model.fit(train_x, train_y)
joblib.dump(model, 'model/lgb_02.pkl')
model = joblib.load('model/lgb_02.pkl')
y_pred = model.predict_proba(test)
y_pred = pd.DataFrame(y_pred, index=test.index)
y_pred = pd.concat([test_ids, y_pred], axis=1)
y_pred.columns = sample.columns
y_pred.to_csv('submission/lgb_02.csv', index=False)
...
model.fit(train_x, train_y, batch_size=100000, validation_split=0.1,
